...
namespace Bit.Notifications
{
    ...
    public class NotificationsHub : Microsoft.AspNetCore.SignalR.Hub
    {
        ...
        public override async Task OnConnectedAsync()
        {
            ...
            {
                foreach (var org in _sessionContext.OrganizationMemberships)
                {
                    await Groups.AddToGroupAsync(Context.ConnectionId, $"Organization_{org.OrganizationId}");
                }
            }
        ...
        public override async Task OnDisconnectedAsync(Exception exception)
        {
            if (_sessionContext.HasOrganizations())
            {
                foreach (var org in _sessionContext.OrganizationMemberships)
                {
                    await Groups.RemoveFromGroupAsync(Context.ConnectionId, $"Organization_{org.OrganizationId}");
                }
            }
